// (removed duplicate, keep only inside Agent class)
...
export class Agent {
	// Save provider config (Groq or Ollama)
	public saveProviderConfig(config: {
		provider: string;
		apiKey: string;
		baseUrl: string;
		model: string;
		...
		// Set up client/model for immediate use
		this.model = config.model;
		if (config.provider === 'Groq') {
			this.setApiKey(config.apiKey);
		} else {
			this.apiKey = config.apiKey;
			this.client = null; // Will use Ollama logic in chat
		}
	}
...
When asked about your identity, you should identify yourself as a coding assistant running on the ${this.model} model via Groq.`;
	...
	public saveApiKey(apiKey: string): void {
		this.configManager.setConfig({apiKey});
		this.setApiKey(apiKey);
	}

	public clearApiKey(): void {
		this.configManager.setConfig({apiKey: undefined});
		this.apiKey = null;
		this.client = null;
	}
	...
	public setModel(model: string): void {
		...
		this.configManager.setConfig({model});
	...
	async chat(userInput: string): Promise<void> {
		...
		// Check provider and branch logic FIRST
		...
		if (config.provider === 'Ollama') {
			...
			// Handle streaming (NDJSON) and non-streaming JSON
			const contentType = response.headers.get('content-type') || '';
			let message = '[No response from Ollama]';
			if (contentType.includes('application/x-ndjson') || contentType.includes('text/event-stream')) {
				// Streaming response: parse line by line and concatenate all content fields
				...
				if (lastChunk.trim()) {
					try {
						const obj = JSON.parse(lastChunk);
						if (obj.message && obj.message.content) {
							fullMessage += obj.message.content;
						}
					} catch (e) {
				...
				message = fullMessage || '[No response from Ollama]';
			} else {
			...
			if (this.onFinalMessage) this.onFinalMessage(message);
			return;
		}

		// Check API key on first message send (Groq only)
		if (!this.client) {
			...
			if (envApiKey) {
				...
				if (config.apiKey) {
					...
					throw new Error(
						'No API key available. Please use /login to set your Groq API key.',
					);
				}
			}
