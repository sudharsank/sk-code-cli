...
export default function PendingToolApproval({
	...
	return (
		<Box flexDirection="column">
			...
			{/* Show key parameters */}
			{(() => {
				const keyParams = formatToolParams(toolName, toolArgs, {
					includePrefix: false,
					separator: ': ',
				});
				return keyParams ? (
			...
			{(toolName === 'create_file' || toolName === 'edit_file') && (
				<Box borderStyle="round" borderColor="yellow" paddingX={1}>
					<DiffPreview toolName={toolName} toolArgs={toolArgs} />
				</Box>
			)}
			...
			<Box flexDirection="column">
				...
				<Box flexDirection="column">
					<Box>
						<Text
							color={selectedApprovalOption === 0 ? 'black' : 'green'}
							backgroundColor={
								selectedApprovalOption === 0 ? 'rgb(124, 214, 114)' : undefined
							}
						>
							{selectedApprovalOption === 0 ? <Text bold>{'>'}</Text> : '  '}{' '}
							Yes
						</Text>
					</Box>
					...
					{!DANGEROUS_TOOLS.includes(toolName) && (
						<Box>
							<Text
								color={selectedApprovalOption === 1 ? 'black' : 'blue'}
								backgroundColor={
							...
							>
								{selectedApprovalOption === 1 ? <Text bold>{'>'}</Text> : '  '}{' '}
					...
					<Box>
						<Text
							color={
								...
								(DANGEROUS_TOOLS.includes(toolName) ? 1 : 2)
									? 'black'
									: 'red'
							}
							backgroundColor={
						...
						>
							...
							No, tell SK what to do differently (esc)
						</Text>
					</Box>
				</Box>
			</Box>
