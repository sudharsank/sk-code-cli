...
export async function createTasks(
	...
	try {
		...
		for (let i = 0; i < tasks.length; i++) {
			...
			if (!task.id || !task.description) {
				return createToolResponse(
					...
					undefined,
					'',
					`Error: Task ${i} missing required fields (id, description)`,
				);
			}
			...
			if (!['pending', 'in_progress', 'completed'].includes(task.status)) {
				return createToolResponse(
					...
					undefined,
					'',
					`Error: Invalid status '${task.status}' for task ${task.id}`,
				);
			}
		}
		...
		return createToolResponse(
			...
			undefined,
			'',
			`Error: Failed to create tasks - ${error}`,
		);
	}
}
	...
	try {
		if (!currentTaskList) {
			return createToolResponse(
				...
				undefined,
				'',
				'Error: No task list exists. Create tasks first.',
			);
		}
		...
		for (const update of taskUpdates) {
			if (!update.id || !update.status) {
				return createToolResponse(
					...
					undefined,
					'',
					'Error: Task update missing required fields (id, status)',
				);
			}
			...
			if (!taskFound) {
				return createToolResponse(
					...
					undefined,
					'',
					`Error: Task '${update.id}' not found`,
				);
			}
		}
		...
		return createToolResponse(
			...
			undefined,
			'',
			`Error: Failed to update tasks - ${error}`,
		);
	}
}
